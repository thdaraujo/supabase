...
[Supabase](https://supabase.com) é uma alternativa ao Firebase de código aberto. Estamos construindo as funcionalidades do Firebase usando ferramentas de código aberto para aplicações em nível corporativo.

- [x] Hospedagem de Banco de dados Postgres. [Docs](https://supabase.com/docs/guides/database)
...
- [Fórum da comunidade](https://github.com/supabase/supabase/discussions). Melhor para: ajuda com desenvolvimento, discussões sobre melhores práticas em banco de dados.
...
- [x] Beta Público: Estável o suficiente para a maioria dos casos não-corporativos.
- [ ] Público: Pronto para ser publicado em produção.

Estamos atualmente na fase Beta Público. Siga os lançamentos (_releases_) deste repositório para receber uma notificação quando uma nova versão for lançada (_major releases_).
...
Supabase é uma combinação de ferramentas de código aberto. Nós estamos construindo as funcionalidades do Firebase usando produtos de código aberto de nível corporativo. Se as ferramentas e comunidades com licenças MIT, Apache 2 ou licenças abertas equivalentes existirem, nós usaremos e apoiaremos tal ferramenta. Se a ferramenta não existir, nós construiremos e abriremos o código nós mesmos. Supabase não possui um mapeamento 1-para-1 das funcionalidades do Firebase. Nosso objetivo é entregar as pessoas desenvolvedoras uma experiência parecida com o Firebase usando ferramentas de código aberto.
...
- [PostgreSQL](https://www.postgresql.org/) é um banco de dados objeto-relational com mais de 30 anos de desenvolvimento ativo que conquistou uma grande reputação por ser confiável, robusto e de ótimo desempenho.
- [Realtime](https://github.com/supabase/realtime) é um servidor Elixir que permite ouvir as inserções, atualizações e remoções (_inserts_, _updates_ e _deletes_) do PostgreSQL usando _websockets_. Supabase utiliza as funcionalidades de replicação nativas do Postgres para receber atualizações do banco de dados, converte essa informação em JSON, e propaga o JSON para outros clientes usando _websockets_.
- [PostgREST](http://postgrest.org/) é um servidor web que converte sua base de dados PostgreSQL diretamente em uma API RESTful.
- [storage-api](https://github.com/supabase/storage-api) oferece uma interface RESTful para gerenciar arquivos armazenados no S3, usando Postgres para gerenciar as permissões.
- [postgres-meta](https://github.com/supabase/postgres-meta) é uma API RESTful para gerenciar seu banco de dados Postgres, permitindo a você carregar tabelas, adicionar permissões, executar consultas e etc.
