...
Supabase Auth provides three sets of APIs that let you add MFA to your
application.

1. **Enrollment API.**  
You can use this to build rich user interfaces for adding and removing factors.
2. **Challenge and Verify APIs.**  
You can use these APIs to securely verify that the user has access to a factor.
3. **List Factors API.**  
You can use these APIs to build rich user interfaces for signing in with
additional factors.
...
Before users can secure their accounts with MFA, you need to provide them with
a UI where they can set up their additional authentication factor. We call this
the enrollment flow.
...
1. Calling the `supabase.auth.mfa.enroll()` API.  
The output of this call is a QR code and a secret. You should display the QR
code to the user and ask them to scan it with their authenticator application.
If they are unable to scan, you can show them the secret in plain text which
they can type up or paste into the app.
...
| Current Level | Possible Level | Meaning                                    |
| -------------:|:-------------- |:------------------------------------------ |
...
function AppWithMFA() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (
          data.possibleLevel === "aal2" &&
          data.possibleLevel !== data.currentLevel
        ) {
  ...
  if (readyToShow) {
    if (showMFAScreen) {
      return <AuthMFA onSuccess={() => setShowMFAScreen(false)} />;
    }
...
```tsx
function AuthMFA({ onSuccess }: { onSuccess: () => void }) {
